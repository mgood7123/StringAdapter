...
#define STRING_ADAPTER_HASHCODE_SPEC_T(TEMPLATE_DEFINITION, T_TEMPLATE_USAGE)  \
    namespace std {                                                            \
        template <TEMPLATE_DEFINITION>                                         \
        struct hash<T_TEMPLATE_USAGE> {                                        \
                size_t operator()(const T_TEMPLATE_USAGE & v) const {          \
                    return v.hashCode();                                       \
                }                                                              \
        };                                                                     \
    }                                                                          \
    namespace std {                                                            \
        template <TEMPLATE_DEFINITION>                                         \
        struct hash<const T_TEMPLATE_USAGE> {                                  \
                size_t operator()(const T_TEMPLATE_USAGE & v) const {          \
                    return v.hashCode();                                       \
                }                                                              \
        };                                                                     \
...
namespace StringAdapter {
    ...
    std::enable_if<std::is_base_of<StringAdapter::BasicStringAdapter<T>,
    adapter_t>::value>::type
    >

    // ...

    // _T is expected for the default char* implementation of classes accepting
    StringAdapter
    //
    // this is optional but highly recommended as ( char* / std::string ) based
    data is extremely common
    //
    //   also QT variants ( QChar / QString ) are very common in QT based
    libs/apps (eg KDE, QTCreator, ect )
    //     so maybe provide a _QT version as well:
    //
    //         using YourClass_QT = YourClass<QChar,
    StringAdapter::CharAdapter_char<QChar, void>>;
    //
    ...
    // TYPEDEF
        ...
        template <
            typename T,
            typename adapter_t,
            typename AdapterMustExtendBasicStringAdapter = typename
    std::enable_if<std::is_base_of<StringAdapter::BasicStringAdapter<T>,
    adapter_t>::value>::type
        >
        // WILL NOT COMPILE
    ...
    adapter_t>::value>::type
        >
        typedef Tuple2<Set<const Named<Patch*, T, adapter_t>>, Set<const
    Named<Patch*, T, adapter_t>>> Deps;
    ...
    adapter_t>::value>::type
        ...
        struct Named<T*, T, adapter_t, typename
    std::enable_if<std::is_base_of<StringAdapter::BasicStringAdapter<T>,
    adapter_t>::value>::type> { Named() { puts("T*");
            }
        };

        // no need to specify Named_T, the above specialization will handle this
    for us
    ...
    */
    ...
    static int compare_3_tuple(const C & a_, const C & b_, const Tuple & t,
    ...
    static int compare_3(const C & a_, const C & b_, const T &... args) {
        return compare_3_tuple(a_, b_, std::tie(args...),
    ...
    template <typename C, class Tuple, std::size_t... Is>
    static std::size_t hash_3_tuple(const C & a_, const Tuple & t,
                                    std::index_sequence<Is...>) {
    ...
    static std::size_t hash_3(const C & a_, const T &... args) {
        return hash_3_tuple(a_, std::tie(args...),
                            std::index_sequence_for<T...> {});
    }

    template <typename C, typename IT_CBEGIN_FUNC, typename IT_CEND_FUNC>
    static int compare_3_iterator(const C & a_, const C & b_,
                                  const IT_CBEGIN_FUNC & b1,
                                  const IT_CEND_FUNC & e1) {
        return lexicographical_compare_2((a_.*b1)(), (a_.*e1)(), (b_.*b1)(),
                                         (b_.*e1)());
    }
...
#define HASHABLE_USING_BASE(BASE) using BASE::hashCode;

    template <typename T>
    class Shared : public Comparable<Shared<T>>, public Hashable<Shared<T>> {
        ...
        public:
            ...
            Shared() :
                Shared<T>::Comparable([](auto & a, auto & b) {
                    return compare_3(a, b, &Shared<T>::ptr_, &Shared<T>::len_);
                }),
                Shared<T>::Hashable([](auto & a) {
                    return hash_3(a, &Shared<T>::ptr_, &Shared<T>::len_);
                }) {}

            Shared(T * ptr, std::size_t len) :
                Shared(ptr, len, [](auto p) {}) {}
            ...
            Shared(const Shared<T> & other) :
                Comparable<Shared<T>>(other), Hashable<Shared<T>>(other) {
                ptr_ = other.ptr_;
                len_ = other.len_;
            }
            Shared(Shared<T> && other) :
                Comparable<Shared<T>>(std::move(other)), Hashable<Shared<T>>(
                                                             std::move(other)) {
    ...
    template <typename T>
    class CShared : public Comparable<CShared<T>>, public Hashable<CShared<T>> {
        ...
        public:
            ...
            CShared() :
                CShared<T>::Comparable([](auto & a, auto & b) {
                    return compare_3(a, b, &CShared<T>::ptr_,
                                     &CShared<T>::len_);
                }),
                CShared<T>::Hashable([](auto & a) {
                    return hash_3(a, &CShared<T>::ptr_, &CShared<T>::len_);
                }) {}

            CShared(const T * ptr, std::size_t len) :
                CShared(ptr, len, [](auto p) {}) {}

            template <class Deleter>
            CShared(const T * ptr, std::size_t len, Deleter deleter) :
                CShared() {
                if (ptr != nullptr) {
            ...
            CShared(const CShared<T> & other) :
                Comparable<CShared<T>>(other), Hashable<CShared<T>>(other) {
                ptr_ = other.ptr_;
                len_ = other.len_;
            }
            CShared(CShared<T> && other) :
                Comparable<CShared<T>>(std::move(other)), Hashable<CShared<T>>(
                                                              std::move(
                                                                  other)) {
                ptr_ = std::move(other.ptr_);
                len_ = other.len_;
...
#define BASIC_STRING_ADAPTER_CSLICE_USING_BASE(BASE)                           \
    ...
    using BASE::get_origin_;
            ...
            class Slice : public Comparable<Slice>, public Hashable<Slice> {
                ...
                public:
                    ...
                    Slice(BasicStringAdapter<T> * origin, std::size_t start,
                          std::size_t end) :
                        origin(origin),
                        start_(start), end_(start < end ? end : start),
                        Slice::Comparable([](auto & a, auto & b) {
                            return compare_3_iterator(a, b, &Slice::cbegin,
                                                      &Slice::cend);
                        }),
                        Slice::Hashable([](auto & a) {
                            return hash_3_iterator<Slice, T>(a);
                        }) {}

                    using iterator =
                        IndexedIterator::iterator<BasicStringAdapter<T>, T>;
                    using reverse_iterator =
                        IndexedIterator::reverse_iterator<BasicStringAdapter<T>,
                                                          T>;

                    using const_iterator =
                        IndexedIterator::iterator<const BasicStringAdapter<T>,
                                                  const T>;
                    using reverse_const_iterator =
                        IndexedIterator::reverse_iterator<
                            const BasicStringAdapter<T>, const T>;
            ...
            class CSlice : public Comparable<CSlice>, public Hashable<CSlice> {
                ...
                public:
                    ...
                    CSlice(const BasicStringAdapter<T> * origin,
                           const std::size_t start, const std::size_t end) :
                        origin(origin),
                        start_(start), end_(start < end ? end : start),
                        CSlice::Comparable([](auto & a, auto & b) {
                            return compare_3_iterator(a, b, &CSlice::cbegin,
                                                      &CSlice::cend);
                        }),
                        CSlice::Hashable([](auto & a) {
                            return hash_3_iterator<CSlice, T>(a);
                        }) {}
            ...
            BasicStringAdapter<T>() :
                BasicStringAdapter<T>::Comparable([](auto & a, auto & b) {
                    return compare_3_iterator(a, b,
                                              &BasicStringAdapter<T>::cbegin,
                                              &BasicStringAdapter<T>::cend);
                }),
                BasicStringAdapter<T>::Hashable([](auto & a) {
                    return hash_3_iterator<BasicStringAdapter<T>, T>(a);
                }) {}
            ...
            using iterator =
                IndexedIterator::iterator<BasicStringAdapter<T>, T>;
            using const_iterator =
                IndexedIterator::iterator<const BasicStringAdapter<T>, const T>;
            ...
            using reverse_iterator =
                IndexedIterator::reverse_iterator<BasicStringAdapter<T>, T>;
            using reverse_const_iterator =
                IndexedIterator::reverse_iterator<const BasicStringAdapter<T>,
                                                  const T>;
            ...
            virtual const CSlice * cslice(std::size_t start,
                                          std::size_t end) const = 0;
            virtual const CSlice * slice(std::size_t start,
                                         std::size_t end) const = 0;
            ...
            virtual const T &
            get_item_at_index(const std::size_t index) const = 0;
            ...
            const int strcmp(const BasicStringAdapter<T> & other) const {
                return lexicographical_compare_2(begin(), end(), other.begin(),
                                                 other.end());
            }
            ...
            const int strcmp(const T * other, const std::size_t len) const {
                return lexicographical_compare_2(
                    begin(), end(),
                    IndexedIterator::ptr_iterator<const T>(other, 0),
                    IndexedIterator::ptr_iterator<const T>(other, len));
            }
            ...
            virtual void insert_(const BasicStringAdapter<T> & what,
                                 const std::size_t pos) = 0;

            virtual void replace_(const BasicStringAdapter<T> & what,
            ...
            const std::size_t clamp_pos(const std::size_t length,
                                        const std::size_t pos) const {
                return pos == -1 ? length : pos >= length ? length : pos;
            }

            const std::size_t clamp_length(const std::size_t clamped_pos,
                                           const std::size_t len) const {
                return clamp_length(size(), clamped_pos, len);
            }

            const std::size_t clamp_length(const std::size_t length,
                                           const std::size_t clamped_pos,
                                           const std::size_t len) const {
                return clamped_pos == length         ? 0
                       : len == -1                   ? length
                       : clamped_pos + len >= length ? length
                                                     : clamped_pos + len;
            }
            ...
            virtual BasicStringAdapter<T> *
            newAdapter(const T * ptr, const std::size_t length) = 0;
            virtual BasicStringAdapter<T> * newAdapter() const = 0;
            virtual BasicStringAdapter<T> * newAdapter(const T * ptr) const = 0;
            virtual BasicStringAdapter<T> *
            newAdapter(const T * ptr, const std::size_t length) const = 0;

            std::vector<std::shared_ptr<BasicStringAdapter<T>>>
            split(const T & splitter) const {
                std::vector<std::shared_ptr<BasicStringAdapter<T>>> vec;
                BasicStringAdapter<T> * c = newAdapter();
                for (const T & t : *this) {
                    if (t == splitter) {
                        vec.push_back(
                            std::shared_ptr<BasicStringAdapter<T>>(c));
                        c = newAdapter();
                    } else {
            ...
            std::vector<std::shared_ptr<BasicStringAdapter<T>>> lines() const {
                ...
                if (len == 0) {
                    return {
                        std::shared_ptr<BasicStringAdapter<T>>(newAdapter())};
                }
...
#define INTERNAL______BASIC_STRING_ADAPTER_USING_BASE_CONSTRUCTOR__TO_SHARED(  \
    ITEM)                                                                      \
    std::shared_ptr<T>(new T(ITEM))

#define BASIC_STRING_ADAPTER_USING_BASE_CONSTRUCTORS(NAME, T)                     \
    NAME(const T & new_line, const T & eof) {                                     \
        init(                                                                     \
            nullptr,                                                              \
            INTERNAL______BASIC_STRING_ADAPTER_USING_BASE_CONSTRUCTOR__TO_SHARED( \
                new_line),                                                        \
            INTERNAL______BASIC_STRING_ADAPTER_USING_BASE_CONSTRUCTOR__TO_SHARED( \
                eof));                                                            \
    }                                                                             \
        ...
        init(                                                                     \
            ...
            INTERNAL______BASIC_STRING_ADAPTER_USING_BASE_CONSTRUCTOR__TO_SHARED( \
                eof));                                                            \
                                                                                  ...
                                                                                  \
    NAME(const T * ptr, const std::size_t length, const T & new_line,             \
         const T & eof) {                                                         \
        init(                                                                     \
            ptr, length,                                                          \
            INTERNAL______BASIC_STRING_ADAPTER_USING_BASE_CONSTRUCTOR__TO_SHARED( \
            ...
            INTERNAL______BASIC_STRING_ADAPTER_USING_BASE_CONSTRUCTOR__TO_SHARED( \
                eof));                                                            \
                                                                                  ...
                                                                                  \
    NAME(const CShared<T> & ptr, const std::size_t length, const T & new_line,    \
         const T & eof) {                                                         \
        init(                                                                     \
            ptr.ptr(), length,                                                    \
            INTERNAL______BASIC_STRING_ADAPTER_USING_BASE_CONSTRUCTOR__TO_SHARED( \
            ...
            INTERNAL______BASIC_STRING_ADAPTER_USING_BASE_CONSTRUCTOR__TO_SHARED( \
                eof));                                                            \
                                                                                  ...
                                                                                  \
    NAME(const std::shared_ptr<const T> & new_line,                               \
         const std::shared_ptr<const T> & eof) {                                  \
        init(nullptr, new_line, eof);                                             \
    }                                                                             \
                                                                                  \
    NAME(const T * ptr, const std::shared_ptr<const T> & new_line,                \
         const std::shared_ptr<const T> & eof) {                                  \
        init(ptr, new_line, eof);                                                 \
    }                                                                             \
                                                                                  \
    NAME(const CShared<T> & ptr, const std::shared_ptr<const T> & new_line,       \
         const std::shared_ptr<const T> & eof) {                                  \
        init(ptr.ptr(), ptr.length(), new_line, eof);                             \
    }                                                                             \
                                                                                  \
    NAME(const T * ptr, const std::size_t length,                                 \
         const std::shared_ptr<const T> & new_line,                               \
         const std::shared_ptr<const T> & eof) {                                  \
        init(ptr, length, new_line, eof);                                         \
    }                                                                             \
                                                                                  \
    NAME(const CShared<T> & ptr, const std::size_t length,                        \
         const std::shared_ptr<const T> & new_line,                               \
         const std::shared_ptr<const T> & eof) {                                  \
        init(ptr.ptr(), length, new_line, eof);                                   \
    }                                                                             \
                                                                                  \
    void init(const T * ptr, const std::shared_ptr<const T> & new_line,           \
              const std::shared_ptr<const T> & eof) {                             \
                                                                                  \
        if (ptr == nullptr) {                                                     \
    ...
    struct ListAdapter : public BasicStringAdapter<T> {
            ...
            ListAdapter(ListAdapter && other) :
                BasicStringAdapter<T>(std::move(other)) {
            ...
            void init(const T * ptr, const std::size_t length,
                      const std::shared_ptr<const T> & new_line,
                      const std::shared_ptr<const T> & eof) {
            ...
            void insert_(const BasicStringAdapter<T> & what,
                ...
                if (vec != nullptr) {
                    ...
                    list.insert(std::next(list.begin(), clamp_pos(pos)),
                                vec->begin(), vec->end() - 1);
                    len += what.length();
                } else {
    ...
    struct ForwardListAdapter : public BasicStringAdapter<T> {
            ...
            const ForwardListAdapter &
            operator=(const ForwardListAdapter & other) {
                BasicStringAdapter<T>::operator=(other);
                forward_list = other.forward_list;
                len = other.len;
                new_line = other.new_line;
            ...
            void insert_(const BasicStringAdapter<T> & what,
                ...
                if (vec != nullptr) {
                    ...
                    forward_list.insert_after(
                        std::next(forward_list.begin(), clamp_pos(pos)),
                        vec->begin(), vec->end() - 1);
                    len += what.length();
                } else {
                    const CShared<T> p = what.data();
...
#define CHAR_ADAPTER_IMPL(ADAPTER)                                             \
    template <typename T>                                                      \
    struct ADAPTER##_char : ADAPTER<T> {                                       \
                                                                               \
            ...
            ADAPTER##_char() : BASE('\n', '\0') {}                             \
                                                                               \
            ADAPTER##_char(const T * ptr) : BASE(ptr, '\n', '\0') {}           \
                                                                               \
            ADAPTER##_char(const T * ptr, const std::size_t length) :          \
                BASE(ptr, length, '\n', '\0') {}                               \
                                                                               \
            ADAPTER##_char(const std::string & str) :                          \
                ADAPTER##_char(str.data(), str.size()) {}                      \
                                                                               \
            ...
            void append(const T * s) {                                         \
                append_(ADAPTER##_char<T>(s, strlen(s)));                      \
            }                                                                  \
                                                                               \
            void append(const T * s, std::size_t len) {                        \
                append_(ADAPTER##_char<T>(s, len));                            \
            }                                                                  \
                                                                               \
            void append(const T & c) {                                         \
                T s[2] = {c, get_end_of_file()};                               \
                append_(ADAPTER##_char<T>(s, 1));                              \
            }                                                                  \
                                                                               \
            void insert(const std::size_t pos, const std::string & s) {        \
                insert_(ADAPTER##_char<T>(s.data(), s.length()), pos);         \
            }                                                                  \
                                                                               \
            void insert(const std::size_t pos, const char * s) {               \
                insert_(ADAPTER##_char<T>(s, strlen(s)), pos);                 \
            }                                                                  \
                                                                               \
            void insert(const std::size_t pos, const char * s,                 \
                        const std::size_t len2) {                              \
                insert_(ADAPTER##_char<T>(s, len2), pos);                      \
            }                                                                  \
                                                                               \
            void insert(const std::size_t pos, const T & c) {                  \
                T s[2] = {c, get_end_of_file()};                               \
                insert_(ADAPTER##_char<T>(s, 1), pos);                         \
            }                                                                  \
                                                                               \
            void replace(const std::size_t pos, const std::size_t len,         \
                         const std::string & s) {                              \
                replace_(ADAPTER##_char<T>(s.data(), s.length()), pos, len);   \
            }                                                                  \
                                                                               \
            void replace(const std::size_t pos, const std::size_t len,         \
                         const char * s) {                                     \
                replace_(ADAPTER##_char<T>(s, strlen(s)), pos, len);           \
            }                                                                  \
                                                                               \
            void replace(const std::size_t pos, const std::size_t len,         \
                         const char * s, const std::size_t len2) {             \
                replace_(ADAPTER##_char<T>(s, len2), pos, len);                \
            }                                                                  \
                                                                               \
            void replace(const std::size_t pos, const std::size_t len,         \
                         const char & c) {                                     \
                T s[2] = {c, get_end_of_file()};                               \
                replace_(ADAPTER##_char<T>(s, 1), pos, len);                   \
            }                                                                  \
                                                                               \
...
\
            ...
            struct CSlice : public BASE::CSlice {                              \
                    ...
                    BASIC_STRING_ADAPTER_SLICE_USING_BASE(BASE_S)              \
                    const ADAPTER##_char<T> * get_origin() override {          \
                        return reinterpret_cast<const ADAPTER##_char<T> *>(    \
                            get_origin_());                                    \
                    };                                                         \
            };                                                                 \
                                                                               \
            Slice * slice(std::size_t start, std::size_t end) override {       \
                return new Slice(this, (begin() + start).index,                \
                                 (begin() + end).index);                       \
            };                                                                 \
                                                                               \
            const CSlice * cslice(std::size_t start,                           \
                                  std::size_t end) const override {            \
                return new CSlice(this, (begin() + start).index,               \
                                  (begin() + end).index);                      \
            };                                                                 \
                                                                               \
            const CSlice * slice(std::size_t start,                            \
                                 std::size_t end) const override {             \
                return new CSlice(this, (begin() + start).index,               \
                                  (begin() + end).index);                      \
            };                                                                 \
                                                                               \
            ADAPTER##_char<T> * clone() override {                             \
                return new ADAPTER##_char<T>(*this);                           \
            }                                                                  \
                                                                               \
            const ADAPTER##_char<T> * clone() const override {                 \
                return new ADAPTER##_char<T>(*this);                           \
            }                                                                  \
                                                                               \
            ADAPTER##_char<T> * newAdapter() override {                        \
                return new ADAPTER##_char<T>();                                \
            }                                                                  \
                                                                               \
            ADAPTER##_char<T> * newAdapter(const T * ptr) override {           \
                return new ADAPTER##_char<T>(ptr);                             \
            }                                                                  \
                                                                               \
            ADAPTER##_char<T> *                                                \
            newAdapter(const T * ptr, const std::size_t length) override {     \
                return new ADAPTER##_char<T>(ptr, length);                     \
            }                                                                  \
                                                                               \
            ADAPTER##_char<T> * newAdapter() const override {                  \
                return new ADAPTER##_char<T>();                                \
            }                                                                  \
                                                                               \
            ADAPTER##_char<T> * newAdapter(const T * ptr) const override {     \
                return new ADAPTER##_char<T>(ptr);                             \
            }                                                                  \
                                                                               \
            ADAPTER##_char<T> *                                                \
            newAdapter(const T * ptr,                                          \
                       const std::size_t length) const override {              \
                return new ADAPTER##_char<T>(ptr, length);                     \
            }                                                                  \
    }
    ...
    using CharVectorAdapter = VectorAdapter_char<char>;
    using CharResizingVectorAdapter = ResizingVectorAdapter_char<char>;
    using CharListAdapter = ListAdapter_char<char>;
    using CharForwardListAdapter = ForwardListAdapter_char<char>;
    ...
    operator<<(::std::ostream & os,
               const StringAdapter::BasicStringAdapter<T> & this_) {
        os << "\"";
        auto s = this_.c_str();
        os << s.ptr();
        os << "\"";
...
STRING_ADAPTER_HASHCODE_SPEC_T(typename T,
                               StringAdapter::BasicStringAdapter<T>);

STRING_ADAPTER_HASHCODE_SPEC_T(typename T, StringAdapter::VectorAdapter<T>);
STRING_ADAPTER_HASHCODE_SPEC_T(typename T, StringAdapter::ListAdapter<T>);
STRING_ADAPTER_HASHCODE_SPEC_T(typename T,
                               StringAdapter::ForwardListAdapter<T>);
STRING_ADAPTER_HASHCODE_SPEC_T(typename T,
                               StringAdapter::ResizingVectorAdapter<T>);

STRING_ADAPTER_HASHCODE_SPEC_T(typename T,
                               StringAdapter::VectorAdapter_char<T>);
STRING_ADAPTER_HASHCODE_SPEC_T(typename T, StringAdapter::ListAdapter_char<T>);
STRING_ADAPTER_HASHCODE_SPEC_T(typename T,
                               StringAdapter::ForwardListAdapter_char<T>);
STRING_ADAPTER_HASHCODE_SPEC_T(typename T,
                               StringAdapter::ResizingVectorAdapter_char<T>);
